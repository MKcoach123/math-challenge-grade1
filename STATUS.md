...
The keys live in the Sheet's `AnswerKey` tab only.
...
- [ ] **Grade 4's `STATUS.md` publishes its full answer-key table** in a public repo — strip it.
- [ ] **Reveal solutions per deadline:** flip `solutions_available` → re-gate → rebuild → push.
- [ ] Per-week **due date** in `week.json` (`"due"`) — currently the `"Sunday night"` placeholder on all 7 weeks.
- [ ] Uncommitted local change: `weeks/grade1/week5/week5_problems.pages` (modified, not yet reviewed).
